...
export default {
	...
	methods: {
				...
				await dishListByCategoryId(param)
					.then(async (res) => {
						if (res && res.code === 1) {
							...
							// 获取每个菜品的月销量
							await this.getMonthlySalesForDishes(this.dishListData)
						}
					})
					.catch((err) => { })
			} else {
		...
		// 获取菜品月销量
		async getMonthlySalesForDishes(dishes) {
			if (!dishes || dishes.length === 0) return
			
			// 为每个菜品获取月销量
			const promises = dishes.map(async (dish) => {
				try {
					const res = await getDishMonthlySales(dish.id)
					if (res && res.code === 1) {
			...
			// 更新视图
			this.dishListData = [...this.dishListData]
		},
